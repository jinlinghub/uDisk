...
typedef struct {
	union {
		I2C_M_SETUP_T    txrx_setup_master;		/* Transmission setup */
		I2C_S_SETUP_T    txrx_setup_slave;		/* Transmission setup */
	};
...
static uint32_t IP_I2C_Start(IP_I2C_001_T *pI2C, IP_I2C_TRANSFER_OPT_T Opt)
{
	uint32_t cnt = 0;
	/* Reset STA, STO, SI */
	pI2C->CONCLR = I2C_I2CONCLR_SIC | I2C_I2CONCLR_STOC | I2C_I2CONCLR_STAC;

	/* Enter to Master Transmitter mode */
	pI2C->CONSET = I2C_I2CONSET_STA;

	if (Opt == I2C_TRANSFER_POLLING) {
		/* Wait for complete */
		while (!(pI2C->CONSET & I2C_I2CONSET_SI)) {
...
static Status IP_I2C_Stop(IP_I2C_001_T *pI2C, IP_I2C_TRANSFER_OPT_T Opt)
{
	uint32_t cnt = 0;
	/* Make sure start bit is not active */
	if (pI2C->CONSET & I2C_I2CONSET_STA) {
		pI2C->CONCLR = I2C_I2CONCLR_STAC;
	}

	pI2C->CONSET = I2C_I2CONSET_STO;

	pI2C->CONCLR = I2C_I2CONCLR_SIC;

	if (Opt == I2C_TRANSFER_POLLING) {
		/* wait for stop is sent */
		while (pI2C->CONSET & I2C_I2CONSET_STO) {
			if (pI2C->CONSET & I2C_I2CONSET_SI) {
				pI2C->CONCLR = I2C_I2CONCLR_SIC;
			}
			if (++cnt > BLOCKING_TIMEOUT) {
				return ERROR;
...
static uint32_t IP_I2C_SendByte(IP_I2C_001_T *pI2C, uint8_t databyte)
{
	uint32_t CodeStatus = pI2C->STAT & I2C_STAT_CODE_BITMASK;
	...
	pI2C->DAT = databyte & I2C_I2DAT_BITMASK;

	pI2C->CONSET = I2C_I2CONSET_AA;
...
static uint32_t IP_I2C_GetByte(IP_I2C_001_T *pI2C, uint8_t *retdat, bool ack)
{
	*retdat = (uint8_t) (pI2C->DAT & I2C_I2DAT_BITMASK);

	if (ack == true) {
		pI2C->CONSET = I2C_I2CONSET_AA;
	}
	else {
		pI2C->CONCLR = I2C_I2CONCLR_AAC;
	}
...
static int32_t IP_I2C_MasterHanleStates(IP_I2C_001_T *pI2C,
										uint32_t CodeStatus,
										...
										IP_I2C_TRANSFER_OPT_T Opt)
{
	...
	case I2C_I2STAT_M_TX_RESTART:
		...
		if (TransferCfg->tx_count < TransferCfg->tx_length) {
			...
			IP_I2C_SendByte(pI2C, (TransferCfg->sl_addr7bit << 1));
			Ret = I2C_BYTE_SENT;
		}
		else if (TransferCfg->rx_count  < TransferCfg->rx_length) {
			/* Send slave address + RD direction bit = 1 ----------------------------------- */
			IP_I2C_SendByte(pI2C, ((TransferCfg->sl_addr7bit << 1) | 0x01));
			Ret = I2C_BYTE_SENT;
		}
	...
	case I2C_I2STAT_M_RX_DAT_ACK:
		if (TransferCfg->rx_count < TransferCfg->rx_length) {
			if ((TransferCfg->rx_length > 1) && (TransferCfg->rx_count < (TransferCfg->rx_length - 2))) {
				IP_I2C_GetByte(pI2C, &tmp, true);
				Ret = I2C_BYTE_RECV;

			}
			else {		/* the next byte is the last byte, send NACK instead */
				IP_I2C_GetByte(pI2C, &tmp, false);
				Ret = I2C_BYTE_RECV;
			}
			*rxdat++ = tmp;
...
/* Handle I2C Master states */
static int32_t IP_I2C_SlaveHanleStates(IP_I2C_001_T *pI2C, uint32_t CodeStatus, I2C_S_SETUP_T *TransferCfg)
{
	int32_t Ret = I2C_OK;
	uint8_t *txdat;
	uint8_t *rxdat;
	...
	case I2C_I2STAT_S_RX_ARB_LOST_M_GENCALL:
		pI2C->CONSET = I2C_I2CONSET_AA | I2C_I2CONSET_STA;
		pI2C->CONCLR = I2C_I2CONCLR_SIC;
		break;
	...
	case I2C_I2STAT_S_RX_PRE_SLA_DAT_ACK:
		...
		if (TransferCfg->rx_count == (TransferCfg->rx_length) ) {
			pI2C->CONCLR = I2C_I2CONCLR_AAC | I2C_I2CONCLR_SIC;
			Ret = I2C_BYTE_RECV;
		}
		else {
			pI2C->CONSET = I2C_I2CONSET_AA;
...
void IP_I2C_Init(IP_I2C_001_T *pI2C)
{
	...
	pI2C->CONCLR = (I2C_I2CONCLR_AAC | I2C_I2CONCLR_SIC | I2C_I2CONCLR_STAC | I2C_I2CONCLR_I2ENC);
}
...
{
	uint32_t returnCode;
	I2C_M_SETUP_T *txrx_setup;
	int32_t Ret = I2C_OK;

	txrx_setup = (I2C_M_SETUP_T *) &i2cdat[I2C_Num].txrx_setup_master;

	while (!(pI2C->CONSET & I2C_I2CONSET_SI)) {}

	returnCode = (uint32_t) (pI2C->STAT & I2C_STAT_CODE_BITMASK);

	/* Save current status */
	txrx_setup->status = returnCode;

	Ret = IP_I2C_MasterHanleStates(pI2C, returnCode, txrx_setup, I2C_TRANSFER_INTERRUPT);
	...
	if (I2C_CheckError(Ret)) {
		if (txrx_setup->retransmissions_count < txrx_setup->retransmissions_max) {
			...
			/* Reset STA, STO, SI */
			IP_I2C_Start(pI2C, I2C_TRANSFER_INTERRUPT);
			return;
		}
		else {
			goto s_int_end;
...
s_int_end:
	...
	pI2C->CONCLR = I2C_I2CONCLR_AAC | I2C_I2CONCLR_SIC | I2C_I2CONCLR_STAC;
...
{
	uint32_t returnCode;
	I2C_S_SETUP_T *txrx_setup;
	int32_t Ret = I2C_OK;

	txrx_setup = (I2C_S_SETUP_T *) &i2cdat[I2C_Num].txrx_setup_slave;
	...
	Ret = IP_I2C_SlaveHanleStates(pI2C, returnCode, txrx_setup);
...
retry:
		...
		/* Start command */
		CodeStatus = IP_I2C_Start(pI2C, I2C_TRANSFER_POLLING);

		while (1) {	/* send data first and then receive data from Slave */
			Ret = IP_I2C_MasterHanleStates(pI2C, CodeStatus, TransferCfg, I2C_TRANSFER_POLLING);
			if (I2C_CheckError(Ret)) {
				...
				if (TransferCfg->rx_count >= (TransferCfg->rx_length)) {
					...
					IP_I2C_Start(pI2C, I2C_TRANSFER_POLLING);
				}
			}
			else if (Ret & I2C_RECV_END) {	/* already receive all data */
				break;
			}
			CodeStatus = pI2C->STAT & I2C_STAT_CODE_BITMASK;
		}
		return SUCCESS;
error:
		return ERROR;
								...
								IP_I2C_TRANSFER_OPT_T Opt)
{
	...
	if (Opt == I2C_TRANSFER_POLLING) {
		...
		pI2C->CONCLR = (I2C_I2CONCLR_SIC | I2C_I2CONCLR_STAC | I2C_I2CONCLR_STOC);
		...
		while (1) {
			...
			if (pI2C->CONSET & I2C_I2CONSET_SI) {
				...
				Ret = IP_I2C_SlaveHanleStates(pI2C, CodeStatus, TransferCfg);
				if (I2C_CheckError(Ret)) {
					goto s_error;
				}
...
s_error:
		...
		/* Enable AA */
		pI2C->CONSET = I2C_I2CONSET_AA;
		pI2C->CONCLR = I2C_I2CONCLR_SIC | I2C_I2CONCLR_STAC;
...
void IP_I2C_SetOwnSlaveAddr(IP_I2C_001_T *pI2C, I2C_OWNSLAVEADDR_CFG_T *OwnSlaveAddrConfigStruct)
{
	...
	case 1:
		pI2C->ADR1 = tmp;
		pI2C->MASK[1] = I2C_I2MASK_MASK((uint32_t) (OwnSlaveAddrConfigStruct->SlaveAddrMaskValue));
		break;

	case 2:
		pI2C->ADR2 = tmp;
		pI2C->MASK[2] = I2C_I2MASK_MASK((uint32_t) (OwnSlaveAddrConfigStruct->SlaveAddrMaskValue));
		break;

	case 3:
		pI2C->ADR3 = tmp;
		pI2C->MASK[3] = I2C_I2MASK_MASK((uint32_t) (OwnSlaveAddrConfigStruct->SlaveAddrMaskValue));
		break;
	}
}
