...
static I2C_M_SETUP_T TransferMCfg;
...
static CHIP_CCU_CLK_T Chip_I2C_DetermineClk(LPC_I2C_T *pI2C) {

	CHIP_CCU_CLK_T i2cclk;
...
static IP_I2C_ID_T Chip_I2C_Get_BusNum(LPC_I2C_T *pI2C)
{
	if (pI2C == LPC_I2C1) {
		return I2C1;
	}
...
{
	/* Enable I2C Clocking */
	Chip_Clock_Enable(Chip_I2C_DetermineClk(pI2C));
...
void Chip_I2C_DeInit(LPC_I2C_T *pI2C)
{
	...
	Chip_Clock_Disable(Chip_I2C_DetermineClk(pI2C));
}
...
void Chip_I2C_SetClockRate(LPC_I2C_T *pI2C, uint32_t clockrate)
{
	...
	IP_I2C_SetClockRate(pI2C, (Chip_Clock_GetRate(Chip_I2C_DetermineClk(pI2C)) / clockrate));
}
...
{
	IP_I2C_ID_T I2C_Num = Chip_I2C_Get_BusNum(pI2C);
	...
	return IP_I2C_MasterTransferData(pI2C, I2C_Num, TransferCfg, Opt);
}
								 ...
								 uint8_t buffer_len)
{
	...
	IP_I2C_MasterTransferData(pI2C, I2C_Num, &TransferMCfg, I2C_TRANSFER_POLLING);
...
void Chip_I2C_Interrupt_MasterHandler(LPC_I2C_T *pI2C)
{
	...
	IP_I2C_Interrupt_MasterHandler(pI2C, I2C_Num);
}
...
Status Chip_I2C_SlaveTransferData(LPC_I2C_T *pI2C, I2C_S_SETUP_T *TransferCfg, IP_I2C_TRANSFER_OPT_T Opt)
{
	...
	return IP_I2C_SlaveTransferData(pI2C, I2C_Num, TransferCfg, Opt);
}
...
void Chip_I2C_Interrupt_SlaveHandler(LPC_I2C_T *pI2C)
{
	...
	IP_I2C_Interrupt_SlaveHandler(pI2C, I2C_Num);
}
